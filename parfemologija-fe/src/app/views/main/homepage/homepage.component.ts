...
@Component({
  ...
  host: {
    class: 'max-body-width m-auto w-full flex flex-col mt-5'
  }
})
export class HomepageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.articleService.getRecentArticles().subscribe({
      next: list => this.recentArticles = list,
    ...
    this.session.fetchUserProfile().subscribe({
      next: user => {
        ...
        this.collectionService.getUserCollections().subscribe({
          next: colls => this.userCollections = colls.slice(0,2)
        });
      }
    });
  }
